...
int main()
{
	...
	{				
		cin >> s1;
		cin >> s2;//getline(cin, s2);
		scanf("%d",&l);
		for(j=1;j<=l;j++)
		{
			c=0;
			list<string> lists1,lists2;
			list<string>::iterator lists1iter,lists2iter;
			for(k=0;k+j<=s1.length();k++)
			{
				lists1.push_back(s1.substr(k,j));
				lists2.push_back(s2.substr(k,j));				
			}		
//			for(k=0;k+j<=s2.length();k++)
//			{
//				lists2.push_back(s2.substr(k,j));
//			}
			for(lists1iter=lists1.begin();lists1iter!=lists1.end();lists1iter++)
				for(lists2iter=lists2.begin();lists2iter!=lists2.end();lists2iter++)
					if((*lists1iter).compare(*lists2iter) == 0)
						c++;
			printf("%d ",c);
		}
		printf("\n");
...
Constraints
    ...
    length of any string ≤ 2*10^5
    In any test file, the sum of lengths of all strings is at most 2*10^6
