...
int main ()
{
	...
	scanf("%d",&t);
        for(i=0;i<t;i++)
        {
        	sum=0;
		scanf("%d",&n);		
		data = (int *) malloc((n+2) * (n+2) * sizeof(*data));
		server = (int **) malloc((n+2) * sizeof(*server));
		for (x = 0; x < (n+2); x++)
		{
			server[x] = &data[x * (n+2)];
		}
        	//server=new int[n+2][n+2];
            	for(j=0;j<=(n+1);j++)
            	{
                	for(k=0;k<=(n+1);k++)
                	{
            	...
            	for(j=1;j<=n;j++)
            	{
                	...
                	{
				//printf("\n%d",server[j][k]);
                    		if(((server[j][k]%2)==0)&&(server[j][k]!=2))
                    		{
                            		sum=sum+server[j][k]/2;
					//printf("\n%d\teven",(server[j][k]/2));
					abc((j-1),k);
					abc((j+1),k);
					abc(j,(k-1));
					...
					{ 
						...
						if(server[j][k]>0)
						{
                               				sum=sum+(server[j][k]/2)+2;
							//printf("\n%d\todd",((server[j][k]/2)+2));
							def((j-1),k);
							def((j+1),k);
							def(j,(k-1));
							def(j,(k+1));
...
{
	if(((server[j][k]%2)==0)&&(server[j][k]!=2)&&(server[j][k]!=-1))
	{
	        server[j][k]=-1;
		abc((j-1),k);
		abc((j+1),k);
...
void def(int j, int k)
{
	if(((server[j][k]%2)!=0)&&(server[j][k]!=-1))
	{
		iter=mymap.find(server[j][k]);
		if(iter == mymap.end())
		{
