...
int main()
{
	...
	map<string, int>::iterator coiter;
	map<string, string>::iterator bt;
	int n,m,i,j,count,l=0,z;
	char nm[100],co[100],vo[100],w[100];
	scanf("%d %d",&n,&m);
	for(i=0;i<n;i++)
	{
		scanf("%s %s",nm,co);
		both.insert ( pair<string,string>(nm,co) );
	}
	for(j=0;j<m;j++)
	{
		scanf("%s",nm);
		...
		if(iter == name.end())
		{
			name.insert ( pair<string,int>(nm,1) );
		}
		else
		{
			count=(*iter).second;
	...
	for (iter = name.begin(); iter != name.end(); iter++)
	{
		...
		{
			l=count;
			strcpy(vo,(*iter).first.c_str());
		}
		bt=both.find((*iter).first);		
		coiter=country.find((*bt).second);
		if(coiter == country.end())
		{
			country.insert ( pair<string,int>((*bt).second.c_str(),(*iter).second) );
		}
		else
		{
			z=(*coiter).second;
			z=z+(*iter).second;
			country[(*bt).second.c_str()]=z;
		}
	}
	l=0;
	for (coiter = country.begin(); coiter != country.end(); coiter++)
	{
		z=(*coiter).second;
		if(z>l)
		{
			l=z;
			strcpy(w,(*coiter).first.c_str());
		}
	}
	printf("\n%s",w);
	printf("\n%s",vo);
