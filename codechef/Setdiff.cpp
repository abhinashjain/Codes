...
int main()
{
	...
	for(i=0;i<t;i++)
	{
        ...
        for(j=1;j<=n;j++)
        {
            tot=(tot+(arr[j-1] * (pow[j-1] - pow[n-j])))%1000000007;
        }
        printf("%lld\n",tot);
    }
	return 0;
...
Churu is working as a data scientist in Coderpur. He works on a lot of data on the daily basis. One day, he found an interesting problem, which was very easy to solve for small data but 
was getting more complex with increasing number of data points. So, Churu needs your help in solving this problem.
...
There might be repeated values in the set. For set S = {1,2,2}, consider that first 2 is not same as the second 2 and there will be two different subsets {1,2}. See last sample case for 
the more clarifications.
